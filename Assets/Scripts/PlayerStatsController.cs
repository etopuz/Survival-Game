...
namespace PlayerScriptSystem
{
    ...
    public class PlayerStatsController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            uiHandler = GameObject.Find("/LifeStatsUI").GetComponent<UIHandler>();
        }

        private void Update()
        {
            isStaminaEnoughForJump = jumpCost <= playerStamina;
            isTired = playerStamina <= LIMIT;
            ControlHunger();
            UpdateSurvivalUI();
            ControlHealth();
            ControlStamina();
        ...
        {
            uiHandler.ChangeHealthImage(playerHealth / maxHealth);
            uiHandler.ChangeHungerImage(playerHunger / maxHunger);
            uiHandler.ChangeStaminaImage(playerStamina / maxStamina);
        }
